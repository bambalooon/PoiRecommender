 ...
 */
public class WeatherTemperatureAdapter extends AbstractNumericStateAdapter<Weather> {
    private static final String WEATHER_TEMPERATURE_ATTRIBUTE = "tempInC";
    private static final long HOUR = 3600000L;
    private static final float FORECAST_OLDEST = 6;
    private static final int NO_CERTAINTY = 0;

    public WeatherTemperatureAdapter(Weather weather) {
        super(weather);
    }

    @Override
    protected String getAttributeName() {
        return WEATHER_TEMPERATURE_ATTRIBUTE;
    }

    @Override
    protected Double adaptValue() {
        final Weather weather = getAdaptee();
        return weather != null ? weather.getTempCurrent() : null;
    }
